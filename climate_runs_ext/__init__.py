...
def load_project_config(config_path=None):
    ...
    # No config.json — return a public-release-shaped empty config. Paper-
    # pipeline usage routes data through the stardust-2d-inputs engine, which
    # has its own config and doesn't need any of these legacy GitHub fields.
    # Code paths that *do* hit the legacy URLs will get a clear None and can
    # raise meaningfully — they shouldn't be reached on the public path.
    if not os.path.isfile(config_path):
        return {
            'proj_name': 'public',
            'aerosols_token': None,
            'climate_database_token': None,
            'optical_table_commit': None,
            'climate_database_commit': None,
            'aerosols_tables_dict': {},
            'aerosols_opt_tables_http': None,
            'climate_database_files_http': None,
            'aerosols_input_dict': {},
        }
